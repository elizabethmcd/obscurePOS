...
melted.names %>% ggplot(aes(x=factor(date), y=diversity, group=code, color=code)) + geom_point() + geom_line() + scale_fill_manual(values=cbPalette) + scale_y_continuous(limits=c(0, .01), expand=c(0,0)) + theme_bw()
